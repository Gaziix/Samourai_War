...
# Icone
icon = pygame.image.load('assets_samourai/samurai_head.png')
...
def launch_game():
    while game.running_samourai == True:
        ...
        pygame.draw.rect(game.screen, game.life_bar.color, game.life_bar.rect_form)
        ...
        if game.player.health >= 100 or game.player.health >= 70:
            game.life_bar.color = (0, 200, 0)
        elif game.player.health < 70 and game.player.health > 30:
            game.life_bar.color = (200, 200, 0)
        elif game.player.health >= 0 and game.player.health <= 30:
            game.life_bar.color = (200, 0, 0)

        # On lance nos projectiles
        game.launch_projectile()
        game.player.all_projectile1.draw(game.screen)  # On affiche nos projectiles qui ont été auparavant lancés
        ...
        vague = random.randint(1,
                               game.monster1.frequency)  # Avec une fréquence qui diminue au fur et à mesure des vaggues
        if vague == 1:
            game.vague()
...
def launch_all():
    ...
    click_sounds = pygame.mixer.Sound("assets_samourai/sounds/click.ogg")
    print("Bienvenue sur Samouraï War !")
    background = pygame.image.load('assets_samourai/bg_cacher.png')
    while running_accueil == True:
        font = pygame.font.SysFont("monospace", 16)
        high_score_display = font.render("High Score : " + str(game.high_scored), 1, (255, 255, 0))
        screen.blit(background, (0, 0))  # mettre un fond d'écran et viser quelle partie on veut
        screen.blit(high_score_display, (0, 0))
        screen.blit(game.accueil.banner, game.accueil.rect_banner)
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # Pour quitter l'application
...
def break_menu():
    ...
    while running == True:
        ...
        screen.blit(high_score_display, (0, 15))
        screen.blit(game.pause.image_pause, game.pause.rect_pause)
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # Pour quitter l'application
                running = False
